    ...
    <nav className="bg-white shadow-md fixed w-full z-20 top-0 left-0 border-b border-gray-200">
      <div className="max-w-screen-xl flex items-center justify-between mx-auto p-4">
        ...
        <div className="hidden md:flex flex-1 justify-center space-x-6 text-sm">
          <Link to="/" className="text-gray-900 hover:text-[#2a7fba]">HOME</Link>
          <Link to="/AllDoctor" className="text-gray-900 hover:text-[#2a7fba]">ALL DOCTORS</Link>
          <Link to="/contact" className="text-gray-900 hover:text-[#2a7fba]">CONTACT</Link>
          <Link to="/about" className="text-gray-900 hover:text-[#2a7fba]">ABOUT</Link>
        </div>
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="relative">
            {loggedInUser ? (
              <img
                src={loggedInUser.profilePic || "https://via.placeholder.com/40"}
                alt="Profile"
                className="w-8 h-8 md:w-10 md:h-10 rounded-full object-cover border border-gray-300 cursor-pointer"
                onClick={() => setIsProfileOpen(!isProfileOpen)}
              />
            ) : (
              <Link to="/Register" className="text-white text-sm p-3 rounded-full bg-[#2a7fba]">
                Create account
              </Link>
            )}
            ...
            {loggedInUser && isProfileOpen && (
              <div
                ...
                onMouseLeave={() => setIsProfileOpen(false)} // Close when mouse leaves
              >
                <Link
                  to="/UserProfile"
      ...
      {isOpen && (
        <div className="md:hidden bg-white w-full p-4 space-y-4 border-t border-gray-200">
          <Link to="/" onClick={closeMenu} className="block text-gray-900 hover:text-[#2a7fba]">
            HOME
          </Link>
          <Link to="/AllDoctor" onClick={closeMenu} className="block text-gray-900 hover:text-[#2a7fba]">
            ALL DOCTORS
          </Link>
          <Link to="/About" onClick={closeMenu} className="block text-gray-900 hover:text-[#2a7fba]">
            ABOUT
          </Link>
          <Link to="/Contact" onClick={closeMenu} className="block text-gray-900 hover:text-[#2a7fba]">
            CONTACT
          </Link>
          <button
